// copy-patse because of some issue with modules
function split2(str, delim) {
    const parts = str.split(delim);
    return [parts[0], parts.splice(1,parts.length).join(delim)];
}

function parseTicket(title) {
    const titleParts = split2(title, ': ')
...
function isSameTicket(title1, title2) {
    return title1 && title2 && (title1 === title2 || parseTicket(title1).id === parseTicket(title2).id)
}


// redmineService
class RedmineService {
    REDMINE_HOST = 'https://redmine.syberry.com/'
    ...
    async getToken() {
        return new Promise(resolve => {
            aBrowser.storage.local.get(['redminescrt'], (result) => resolve(result['redminescrt']))
        })
    }
...
const getSettings = () => {
    const isMoveInProgressEnabled = async () => {
        return new Promise(resolve => {
            aBrowser.storage.local.get(['__davinci_settings'], result => {
                const settings = JSON.parse(result['__davinci_settings'] || '{}')
                resolve(settings.isMoveInProgress)
            })
        })
    ...
    return {
        isMoveInProgressEnabled
    }
}

const redmineService = new RedmineService();

const moveInProgressIfNeeded = async (timeEntry) => {
    const {id} = parseTicket(timeEntry.description)
    const token = await redmineService.getToken();
    const settings = getSettings();
    const isMoveInProgressEnabled = await settings.isMoveInProgressEnabled();

    if (isMoveInProgressEnabled && id && token) {
        const issue = await redmineService.getById(id)
        console.info('Status:  ', issue.status)

        if (issue && issue.status.id === redmineService.Status.NEW) {
            console.info('Setting in progress status for ' + id)
            await redmineService.update(id, {
                status_id: redmineService.Status.IN_PROGRESS
            })
...
var clockifyButton = {
    ...
    createButton: (description, project, task) => {
        ...
        aBrowser.storage.local.get(["timeEntryInProgress"], (result) => {
            ...
            let active = title && (
              isSameTicket(title, clockifyButton.inProgressDescription)
            );
...
function updateButtonState(entry) {
    ...
    for (let i = 0; i < document.clockifyButtonLinks.length; i++) {
        ...
        const active = entry && isSameTicket(button.title, entry.description)
...
function buttonClicked(button, options) {
    ...
    if (title && isSameTicket(title, clockifyButton.inProgressDescription)) {
        ...
        aBrowser.runtime.sendMessage({
            ...
            if (response.status === 400) {
                ...
                moveInProgressIfNeeded(response.data)
